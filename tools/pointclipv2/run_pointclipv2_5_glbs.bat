...
for %%M in (a b c o1778 o3049) do (
  echo.
  echo ===== PointCLIPV2 zero-shot %%M.glb =====
  python %PCV2%\run_pointclipv2_zeroshot_glb.py ^
    ...
    --batch-size 8 ^
    --top-k 5 ^
    --output-csv %OUT%\%%M_pointclipv2_zeroshot.csv ^
    --output-json %OUT%\%%M_pointclipv2_zeroshot.json ^
    --summary-csv %OUT%\%%M_pointclipv2_summary.csv
)
